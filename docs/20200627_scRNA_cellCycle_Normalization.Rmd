...
In this note we examplify the application of MetaCell package to identify gene co-varition in scRNA-seq data of the HCT116 colorectal cancer cell-line, which is indepent of the cell-cycle.

Cell-cycle associated expression is perhaps the most common feature to dominate transcriptional heterogeneity in scRNA databases (at least those that cover proliferative cell populations). Whereas the cell-cycle machniery is largely conserved, its transcrtiptional signature can vary greatly between different biological systems, mainly due to different replication time-scales and different fractions of proliferating cells.

So why not simply normalize it out? One common routine of exploring transctiptional heterogeinity in single cell RNA-seq data is finding gene correlation in normalized expression matrices. The main issue with crude normalization of such datasets are that raw UMI counts are sparse, and scaling them by any model which is predicting background trends introduces strong biases. The cycling of a cell is inherently linked to its mRNA content, and therefore is highly susecptible for various biases during normaliztion of raw UMI counts.

In this vignette we wished to avoid normalization of gene counts and then computing of their correlation scores, and developed a startegy that is using the Metacell analytic platform (Baran et al., Genomee bio. 2019) to estimate to the degree of correlation between any gene pair and test how much of this correlation stems from cell-cycle trends. 

We can summarize the strategy we used to achieve this into two main steps:
  ...
  2. Generate a randomized UMI vector for each cell, by sampling each gene's expression from one of its K neareast cells, as defined by the clean cell-cycle model. Gene-to-gene correlations between such randomizeded vectors are expected to preserve cell cycle related gene modules, but to completely, or almost completely eliminate correlations between genes that are co-expressed or co-repressed in a cell cycle independent fashion. 

But little steps for little feet. let's start by installing MetaCell as described here:
...
We can now begin work on our cell-cycle model. A good starting point will be to fish out genes that have significant correlation with at least one strong cell-cycle "anchors" (i.e., MKI67, PTTG, GINS2, ACTB),and optionally, NOT correlated significantly to any gene that is a-priori assumed to be unrelated to cell-cycle (ANXA1). in order to do so, we will create a .txt table, summarizing genes that are correlated to our anchors:
...
For crude analysis, let's take all genes that had a siginficant correlation with at least one of our anchors (so their names appear in the .txt table we have just saved), and create a MeatCell "geneSet" consiting feature genes that will help us to create the cell-cycle model:
...
This will create a subfolder **hct116_cellCycle.gset_cors** in our figures directory, where gene-gene correlations of the *K* module will be displayed, as well as each gene correlation to total UMI per cell. go over and examine the modules, and annotate those that smell like cell-cycle.
...
Reagrdless of whether we decided to use all cell-cycle associated geneSet (**"hct116_cellCycle"**, which will be used in this example) or a purified subset of it (**"hct116_cellCycle_narrow"**), we can now create a balanced KNN similarity graph between cells that is based on expression coming from these genes only:
...
There are many different ways to approach this, but since we are using the MetaCell platform, it just make sense to create metacells that are based on our cell graph, and test whether we can position metacells along a continuum of transcriptional cell-cycle dynamics. Remember in most cases we may only succeed to model the fraction of cycling ones, which can vary between biological samples (the non-cycling cells might form a big "non-cycling" metacell). In addition, we can expect that different distributions of total mRNA content per cell to be one hallmark of our cell-cycle based metacells (but not neccassaarily, especially if sequencing of libraries is not at saturation or nearing it): 
...
For more functions and details on plotting metacells, checkout Metacell github:
https://github.com/tanaylab/metacell
...
the list object **gene_cors_cellCycle** now holds two equally sized gene-gene correlation matrices - **g_cor** and **r_cor**, which will contain the correlation scores between and after the randomization process, respectively.
  ...
  - If the coexpression of two genes is related to cell-cycle, than we could expect that some degree of covariance between them will survive the randomization
  - However, if coexpression patterns are unrelated to cell-cycle, correlations between genes are expected to be completely eliminated after the randomzation, (statstically, if K is large enough).

We can have a supervised look into the two matrices and see whether a specific gene-gene interaction that is intersting for us was eliminated or not.   
...
an example for how such plot can be interperted:
...
still, it doesn't mean that all their transcriptional covariation are fully dependent on the cell-cyle.

in order to have a more careful look at the cel-cycle based randomization on gene correlations, we can plot a "chimeric"" matrix (for selected genes, as in this example), comparing the original correlation value in its top triangle, and the shuffled correlation scores in the lower triangle:
